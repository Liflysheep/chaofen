...
class betapan(object):
    ...
    def generate_hrhsi(self, save_dir, dir_name):
        """
        生成高分辨率高光谱图像并保存为 .mat 文件。
        :param save_dir: 保存目录。
        :param filename: 模型检查点路径。
        """
        # 生成高分辨率高光谱图像
        gen_hrhsi = self.gen_hrhsi(self.hr_msi, reuse=True)
        feed_dict = self.next_feed()

        # 加载模型
        saver = tf.train.Saver()
        save_path = tf.train.latest_checkpoint(dir_name)
        ...
        # 保存为 .mat 文件
        mat_path = os.path.join(save_dir, 'HR.mat')
        sio.savemat(mat_path, {'HR': hrhsi})
        print("High-resolution HSI saved at: %s" % mat_path)
