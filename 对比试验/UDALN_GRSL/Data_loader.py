...
class Dataset(data.Dataset):
    def __init__(self, args):
        ...
        for i in range(len(self.imgpath_list)):
            #选择HR
            self.img_list.append(io.loadmat(self.imgpath_list[i])['HR'])#
    ...
    def get_spectral_response(self,data_name):
        if 'toy' in data_name:
            xls_path = os.path.join(self.args.sp_root_path, 'toys.xls')
        elif data_name == 'Chikusei':
            xls_path = os.path.join(self.args.sp_root_path, 'TG.xls')
        else:
