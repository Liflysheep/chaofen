...
def process_folders(hr_folder, out_folder, output_csv):
    ...
    for hr_prefix, out_subdirs in out_subfolders.items():
        ...
        for out_subdir, scale in out_subdirs:
            ...
            if 'Out' not in out_data:
                print(f"Warning: 'Out' key not found in {out_file}.")
                continue

            out = out_data['Out']
            out = out.transpose(2, 0, 1)  # 调整维度顺序

            # 计算指标
            psnr, sam, ergas, ssim_val, uqi_val = MetricsCal(gt, out, scale)

            # 将结果存入 DataFrame
            # results.loc[f'{hr_prefix}_{scale}'] = [psnr, sam, ergas, ssim_val, uqi_val]
...
out_folder = './UDALN_GRSL/checkpoints'   # OUT 文件夹路径
output_csv = './UDALN_GRSL_results.csv'  # 输出 CSV 文件路径
